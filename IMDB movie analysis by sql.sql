...
select count(*) as row_count_names from names;
select count(*) as row_count_ratings from ratings;
select count(*) as row_count_movie from movie;
select count(*) as row_count_genre from genre;
select count(*) as row_count_director_mapping from director_mapping;
select count(*) as row_count_role_mapping from role_mapping;
...
select g.genre, count(m.id) as num_of_movie from genre g
inner join movie m on g.movie_id = m.id
group by genre order by count(id) desc limit 1;
...
from genre group by movie_id having number_of_movies=1)
...
group by genre order by avg(m.duration) desc;
...
select * from ct_genre_rank where genre="thriller";
...
rank() over(order by count(d.movie_id) desc) director_rank from names n
...
select director_name, movie_count from top_director
...
GROUP BY name
HAVING COUNT(r.movie_id) >= 5 limit 1;
...
GROUP BY n.name HAVING COUNT(m.id) >=3 LIMIT 5;
...
group by name HAVING COUNT(m.id) >=3 limit 3;
...
from top_director_summary group by director_id 
